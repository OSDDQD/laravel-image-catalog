                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group{{ $errors->first('category_id') ? ' has-error' : null }}">
                            ...
                            <select class="form-control position-parent" id="category_id" name="category_id">
                                @foreach (Catalog\Category::with('translations')->get() as $category)
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <img src="{{ \URL::Route('preview.managed', ['object' => 'album', 'mode' => 'preview', 'format' => 'jpg', 'file' => $entity->image]) }}" alt="" />
                        </div>
                    </div>
...
<script>
    $(function() {
        managePositions(
            '{{ json_encode(Catalog\Album::getByParentList(["parentField" => "category_id"])) }}',
            {{ $entity->id ? (int) $entity->category_id : 'undefined' }},
            {{ $entity->id ? 1 : 0 }},
            '{{ Lang::get("forms.labels.new_album") }}',
